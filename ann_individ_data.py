...
def main():
    ...
    for iteration in range(num_iterations):
        for data in range(X.shape[1]):
            ...
            cqm.add_constraint(
                ((input_1 * w['level1']['w11'] + input_2 * w['level1']['w21'] + w['level1']['b1']) * qw[0] +
                 (input_1 * w['level1']['w12'] + input_2 * w['level1']['w22'] + w['level1']['b2']) * qw[1] + qw[2] - output_y) >= 0.0, label='C'
            )
            w = update_weights(X, Y, cqm, w, variable_level)
            print('new error: ', run_ann(X, Y, w), '\n')

            ######### BACK PROPAGATION - PHASE 2 ##########
            print('ITERATION ', iteration + 1,'. PHASE 2:')
            ...
            cqm = dimod.ConstrainedQuadraticModel()
            obj_fun = (
                (input_1 * qw[0] + input_2 * qw[1] + qw[2]) * w['level2']['w31'] +
                (input_1 * qw[3] + input_2 * qw[4] + qw[5]) * w['level2']['w41'] + w['level2']['b3'] - output_y
            )
            cqm.set_objective(obj_fun)
            cqm.add_constraint(
                ((input_1 * qw[0] + input_2 * qw[1] + qw[2]) * w['level2']['w31'] +
                (input_1 * qw[3] + input_2 * qw[4] + qw[5]) * w['level2']['w41'] + w['level2']['b3']) - output_y >= 0.0, label='C'
            ) 
            w = update_weights(X, Y, cqm, w, variable_level)
            print('new error: ', run_ann(X, Y, w), '\n')
